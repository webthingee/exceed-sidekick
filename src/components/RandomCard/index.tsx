...
const imageUrl = (c: Card) =>
  `https://level99games.com/cards/${c.set}/${c.num}.jpg`;
...
export default function RandomCard(): ReactNode {
  ...
  return (
    <section className={styles.wrap} aria-label="Random Exceed Fighting System card">
      <a
        className={styles.card}
        href={`https://level99games.com/?s=${encodeURIComponent(card.name)}`}
        target="_blank"
        rel="noopener noreferrer"
        title={`Look up “${card.name}” on level99games.com`}>
        <img
      ...
      <div className={styles.meta}>
        ...
        <p className={styles.credit}>
          Card images via <Link href="https://level99games.com">level99games.com</Link>
        </p>
      </div>
    </section>
  );
